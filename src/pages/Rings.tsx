...
export default function Rings() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      toast({
        title: "Erro",
        description: "Não foi possível carregar os anéis",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12 py-16 bg-gradient-luxury rounded-lg">
          ...
          <p className="text-white/80 text-lg max-w-2xl mx-auto">
            Descubra nossa coleção de anéis únicos, cada peça criada para celebrar momentos especiais
          </p>
        </div>

        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-2xl font-serif text-foreground">Coleção de Anéis</h2>
            <p className="text-muted-foreground">
              {products.length} anel{products.length !== 1 ? 'is' : ''} encontrado{products.length !== 1 ? 's' : ''}
            </p>
          </div>
